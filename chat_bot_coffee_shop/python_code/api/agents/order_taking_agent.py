...
class OrderTakingAgent():
    ...
    def get_response(self, messages):
        ...
        # Reconstruct last order state from memory
        last_order_taking_status = ""
        asked_recommendation_before = False
        for msg in reversed(messages[:-1]):
            if msg.get("role") == "assistant" and msg.get("memory", {}).get("agent") == "order_taking_agent":
                last_order_taking_status = (
                    f"step number: {msg['memory'].get('step number')}\n"
                    f"order: {msg['memory'].get('order')}\n"
                )
                asked_recommendation_before = msg['memory'].get('asked_recommendation_before', False)
                break
        ...
        # Call the LLM
        raw_output = get_chatbot_response(self.client, self.model_name, input_messages)
        # Ensure JSON validity
        corrected_output = double_check_json_output(self.client, self.model_name, raw_output)
        # Debug: show what arrives
        print("OrderTaking RAW output:", repr(corrected_output))

        return self.postprocess(corrected_output, messages, asked_recommendation_before)

    def postprocess(self, output, messages, asked_recommendation_before):
        # Fallback payload
        if payload.get("order"):
            print("⚠️ OrderTakingAgent: respuesta vacía, usando fallback")
            payload = {"chain of thought": "", "step number": 1, "order": [], "response": "Lo siento, hubo un error procesando tu pedido."}
        else:
            try:
                payload = json.loads(output)
            except json.JSONDecodeError as e:
                print("⚠️ OrderTakingAgent JSONDecodeError:", e)
                try:
                    payload = json.loads(double_check_json_output(self.client, self.model_name, output))
                except Exception as e2:
                    print("❌ Sigue inválido tras corrección:", e2)
                    payload = {"chain of thought": "", "step number": 1, "order": [], "response": "Lo siento, no pude entender tu pedido."}

        # If order is a nested string, parse it
        if isinstance(payload.get("order"), str):
            try:
        ...
        if not asked_recommendation_before and payload.get("order"):
            rec_out = self.recommendation_agent.get_recommendations_from_order(messages, payload["order"])
            response_text = rec_out.get('content', response_text)
        ...
        return {
            ...
            "memory": {
                ...
                "step number": payload.get("step number", 1),
